...
def train(
    ...
    for epoch in range(epochs):
        ...
        import subprocess
        cmd_flickr = [
            "bash", "scripts/eval_flickr30k_new.sh",
            "train_flickr30k",
            str(0),
        ...
        # result = subprocess.run(cmd_flickr, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        cocoinfo = subprocess.run(cmd_coco, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        # # 3. 打印脚本输出
...
def main():
    ...
    parser.add_argument('--path_of_datasets', default = f'/home/liu/.workspace/IFCap/annotations/coco/coco_with_entities_cocoid_imgfeature.pickle') # coco_with_entities
