...
def main():
    ...
    if args.frozen1 or args.frozen2:
        ...
        # ori_model = ClipCaptionModelOri(args, args.continuous_prompt_length, args.clip_project_length, clip_hidden_size, args.num_layers, gpt_type = args.language_model, soft_prompt_first = args.soft_prompt_first, only_hard_prompt = args.only_hard_prompt)
            ori_model_state_dict = torch.load("/home/liu/.workspace/IFCap/checkpoints_r32_alpha64_1_1/train_msrvtt/0020.pt")
        elif "sydney" in args.rt_path:
            ori_model_state_dict = torch.load("/home/liu/.workspace/IFCap/checkpoints_r32_alpha64_1_0/train_msvd/0010.pt")
        
        # if args.frozen2:
        #     for i in range(12):
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.attn.c_attn.base_layer.weight'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.attn.c_attn.base_layer.weight"]
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.attn.c_attn.base_layer.bias'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.attn.c_attn.base_layer.bias"]
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.attn.c_proj.base_layer.weight'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.attn.c_proj.base_layer.weight"]
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.attn.c_proj.base_layer.bias'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.attn.c_proj.base_layer.bias"]
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.mlp.c_proj.base_layer.weight'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.mlp.c_proj.base_layer.weight"]
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.mlp.c_proj.base_layer.bias'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.mlp.c_proj.base_layer.bias"]
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.mlp.c_fc.base_layer.weight'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.mlp.c_fc.base_layer.weight"]
        #         ori_model_state_dict[f"'gpt.base_model.model.transformer.h.{i}.mlp.c_fc.base_layer.bias'"] = ori_model_state_dict[f"gpt.base_model.model.transformer.h.{i}.mlp.c_fc.base_layer.bias"]
        
        # if args.frozen1 and args.use_moe_lora:    
        #     for i in range(8):
        #         ori_model_state_dict[f"mapping_network.transformer.layers.{i}.attn.to_queries.base_layer_.weight"] = ori_model_state_dict[f"mapping_network.transformer.layers.{i}.attn.to_queries.base_layer_.weight"]
        #         # ori_model_state_dict.pop(f"mapping_network.transformer.layers.{i}.attn.to_queries.base_layer_.weight")
        #         ori_model_state_dict[f"mapping_network.transformer.layers.{i}.attn.to_keys_values.base_layer_.weight"] = ori_model_state_dict[f"mapping_network.transformer.layers.{i}.attn.to_keys_values.base_layer_.weight"]
        #         # ori_model_state_dict.pop(f"mapping_network.transformer.layers.{i}.attn.to_keys_values.weight")    

            
        #     ori_model_state_dict["mapping_network.crossatt.transformer.layers.0.attn.to_queries.base_layer_.weight"] = ori_model_state_dict["mapping_network.crossatt.transformer.layers.0.attn.to_queries.base_layer_.weight"]
        #     # ori_model_state_dict.pop("mapping_network.crossatt.transformer.layers.0.attn.to_queries.weight")
        #     ori_model_state_dict["mapping_network.crossatt.transformer.layers.0.attn.to_keys_values.base_layer_.weight"] = ori_model_state_dict["mapping_network.crossatt.transformer.layers.0.attn.to_keys_values.base_layer_.weight"]
        #     # ori_model_state_dict.pop("mapping_network.crossatt.transformer.layers.0.attn.to_keys_values.weight")    
        #     ori_model_state_dict["mapping_network.linear.base_layer_.weight"] = ori_model_state_dict["mapping_network.linear.base_layer_.weight"]
        #     #ori_model_state_dict.pop("mapping_network.linear.weight")    
        #     ori_model_state_dict["mapping_network.rt_linear.base_layer_.weight"] = ori_model_state_dict["mapping_network.rt_linear.base_layer_.weight"]
        ...
        model.load_state_dict(ori_model_state_dict)
