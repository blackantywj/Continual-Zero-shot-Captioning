...
class ClipCaptionModel(nn.Module):
    ...
    def __init__(
        ...
        if gpt_type == "gpt2":
            self.embed_tokens = self.gpt.transformer.wte
            self.denoise_model = self.gpt.transformer # use inputs_embeds instead of input_ids in forward function
            for gpt2block in self.gpt.transformer.h:
