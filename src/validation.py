...
def main(args) -> None:
    ...
    if args.name_of_datasets == 'nocaps': # nocaps
        ...
        if args.using_image_features:
            validation_msvd_msrvtt(args, inpath, entities_text, texts_embeddings, model, tokenizer, preprocess, encoder)
        else:
            validation_msvd_msrvtt(args, inpath, entities_text, texts_embeddings, model, tokenizer, preprocess, encoder)        
    elif args.name_of_datasets in ['sydney', "ucm"]:
        if args.using_image_features:
            validation_sydney_ucm(args, inpath, entities_text, texts_embeddings, model, tokenizer, preprocess, encoder)
        else:
            validation_sydney_ucm(args, inpath, entities_text, texts_embeddings, model, tokenizer, preprocess, encoder)        
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_moe_lora', type=bool, default=False)
    parser.add_argument('--r', type=int, default=32)
    parser.add_argument('--lora_alpha', type=int, default=64)
    parser.add_argument('--frozen1', type=int, default=0)
    parser.add_argument('--frozen2', type=int, default=0)
