...
def validation_coco_flickr30k(
    ...
    with open("true_label_group2_flickr30k_msvd_sydney.json", 'r') as file:
        ftm = json.load(file)
    
    ftm = ftm[args.path_of_val_datasets.split("/")[1]]      
...
def main(args) -> None:
    ...
    modeldict = torch.load(args.weight_path, map_location = device)
    model.load_state_dict(modeldict, strict=False)
    model.to(device)

    modelnew1 = ClipCaptionModel(args, args.continuous_prompt_length, args.clip_project_length, 512, gpt_type = args.language_model, k=args.k)
    modelnew1dict = torch.load("/home/liu/.workspace/IFCap/checkpoints_r32_alpha64_1_0/train_msvd/0010.pt")
    modelnew1dict['mapping_network.prefix_const'] = modeldict["mapping_network.prefix_const"]
    modelnew1.load_state_dict(modelnew1dict, strict=False)
    modelnew1.to(device)
    ...
    modelolddict = torch.load("/home/liu/.workspace/IFCap/checkpoints_r32_alpha64_0_0/train_flickr30k/009.pt")
    modelolddict['mapping_network.prefix_const'] = modeldict["mapping_network.prefix_const"]
    modelold.load_state_dict(modelolddict, strict=False)
    modelold.to(device)
    ...
    if args.name_of_datasets == 'nocaps': # nocaps
        ...
        if args.using_image_features:
            validation_coco_flickr30k(args, inpath, entities_text, texts_embeddings, model, modelnew1, modelold, tokenizer, preprocess, encoder)
        else:
            validation_coco_flickr30k(args, inpath, entities_text, texts_embeddings, model, modelnew1, modelold, tokenizer, preprocess, encoder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name_of_datasets', default = 'nocaps', choices = ('coco', 'flickr30k', 'nocaps', 'sydney')) # coco
