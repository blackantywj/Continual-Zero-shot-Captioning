...
def extract_cocoid(filename: str) -> int:
    """
    从 COCO 图片文件名中提取 cocoid。
    
    参数:
        filename: 包含路径的图片文件名，例如 "COCO_train2014_000000123456.jpg" 或 "/path/to/COCO_val2014_000000654321.jpg"
    
    返回:
        文件名中提取出的 cocoid（整数类型）
    
    异常:
        如果文件名中没有找到合法的 cocoid，则会抛出 ValueError。
    """
    # 获取不含路径的文件名
    basename = os.path.basename(filename)
    # 去掉扩展名
    name, _ = os.path.splitext(basename)
    # 假设 cocoid 是文件名中最后一个下划线后的数字部分
    cocoid_str = name.split('_')[-1]
    try:
        return int(cocoid_str)
    except ValueError:
        raise ValueError(f"无法从文件名 '{filename}' 中提取有效的 cocoid")
...
class CaptionsDataset(Dataset):
    ...
    def __init__(
        ...
        # with open("annotations/coco/coco_with_entities_cocoid_imgfeature.pickle", 'rb') as infile: # loading datasets
        #     all_captions_with_entities = pickle.load(infile)
        ...
        self.rt_captions = []
        ## source code ##
        # with open("coco_train_vitb32.pickle", 'rb') as f:
        #     rt_imgs = pickle.load(f)
        # rt_imgs = {k:v for k, v in sorted(rt_imgs.items(), key = lambda x: (int(x[0].split("_")[0]), int(x[0].split('_')[1][0])))}
        # syn_imgs = {}
        # for key in rt_imgs.keys():
        #     syn_imgs[key] = rt_imgs[key]
        # for key in rt_caps.keys():
        #     rt_caps[key] = rt_caps[key][1][:args.k]

        self.rt_captions = []   
        self.syn_image_features = [] 
        # cap_feat = {c.rstrip():f for e, c, f, g, h in captions_with_entities}
        all_cap_feat = {c.rstrip():f for e, c, f, g, h in captions_with_entities}
        
        for caption_with_entities in captions_with_entities:
            if self.using_clip_features:
                temp_detected_entities, temp_caption, temp_clip_features, _, temp_img_fearures = caption_with_entities
                self.captions_clip_features.append(temp_clip_features)   # dtype = float16, size = (clip_hidden_size, )
                self.rt_captions.append(list(map(all_cap_feat.get, rt_caps[temp_caption.rstrip()])))
                self.syn_image_features.append(temp_img_fearures)
            else:
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        """
        # print(self.rt_captions[242504])
        # print(self.syn_image_features[242504])
        ...
        if self.using_clip_features:
            captions_clip = self.syn_image_features[item]
        else:
