...
def train(
    ...
    for epoch in range(epochs):
        ...
        import subprocess
        cmd = [
            "bash", "scripts/eval_flickr30k_816.sh",
            "train_flickr30k",
            str(0),
        ...
        _ = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
...
def main():
    ...
    parser.add_argument('--path_of_datasets', default = f'/home/cumt/workspace/IFCap/annotations/flickr30k/flickr30k_texts_features_ViT-B32_cocoid_imgfeature.pickle') # coco_with_entities
    ...
    ori_model_state_dict = torch.load("checkpoints_img_fea_best_1098_norm_prefix_0/coco/coco_prefix-004.pt")
    for i in range(12):
    ...
    args.out_dir = f"{args.out_dir.split('/')[0]}_lorafull_r{args.r}_alpha{args.lora_alpha}/{args.out_dir.split('/')[1]}"
