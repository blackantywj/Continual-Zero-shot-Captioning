...
def validation_coco_flickr30k(
    ...
    modelnew1: ClipCaptionModel,                 # trained language model
    ...
    with open("true_label_group1_coco_msrvtt_ucm.json", 'r') as file:
        ftm = json.load(file)

    ftm = ftm[args.name_of_datasets] 
...
def main(args) -> None:
    ...
    modelnew.to(device)
    modelnew1 = ClipCaptionModel(args, args.continuous_prompt_length, args.clip_project_length, clip_hidden_size, gpt_type = args.language_model, k=args.k)
    modelnew1.load_state_dict(torch.load("/home/liu/.workspace/IFCap/checkpoints_r32_alpha64_1_1/train_msrvtt/0020.pt", map_location = device), strict=False)
    modelnew1.to(device)
    ...
    if args.name_of_datasets == 'nocaps': # nocaps
        if args.using_image_features:
            validation_nocaps(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)
        else:
            validation_nocaps(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)
    elif args.name_of_datasets in ['coco', "flickr30k"]: # coco, flickr30k
        if args.using_image_features:
            validation_coco_flickr30k(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)
        else:
            validation_coco_flickr30k(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)
    elif args.name_of_datasets in ['msvd', "msrvtt"]:
        if args.using_image_features:
            validation_msvd_msrvtt(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)
        else:
            validation_msvd_msrvtt(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)        
    elif args.name_of_datasets in ['sydney', "ucm"]:
        if args.using_image_features:
            validation_sydney_ucm(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)
        else:
            validation_sydney_ucm(args, inpath, entities_text, texts_embeddings, modelnew, modelnew1, modelold, tokenizer, preprocess, encoder)        
