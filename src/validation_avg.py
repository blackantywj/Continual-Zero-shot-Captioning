...
from ClipCapMemoryLora import ClipCaptionModel
from ClipCap import ClipCaptionModel as ClipCaptionOriginModel
...
with open("/home/liu/.workspace/IFCap/annotations/all_datasets_centroidsk100_img.pickle", 'rb') as file:
    avgdict_img = pickle.load(file)
    
with open("/home/liu/.workspace/IFCap/annotations/all_datasets_centroidsk100_txt.pickle", 'rb') as file:
    avgdict_txt = pickle.load(file)
# with open("annotations/coco/tasktoavgemb.pickle", 'rb') as f:
...
avg_coco_txt = torch.from_numpy(avgdict_txt['coco'])
avg_msrvtt_txt = torch.from_numpy(avgdict_txt['msrvtt'])
avg_ucm_txt = torch.from_numpy(avgdict_txt['ucm'])
avg_coco_img = torch.from_numpy(avgdict_img['coco'])
avg_msrvtt_img = torch.from_numpy(avgdict_img['msrvtt']) 
avg_ucm_img = torch.from_numpy(avgdict_img['ucm'])
avgall1 = torch.stack([avg_coco_txt, avg_msrvtt_txt])
avgall2 = torch.stack([avg_coco_img, avg_msrvtt_img])
...
def validation_coco_flickr30k(
    ...
    entities_text: List[str],                # entities texts of vocabulary
    texts_embeddings: torch.Tensor,          # entities embeddings of vocabulary
    modelnew: ClipCaptionModel,                 # trained language model   
    modelold: ClipCaptionOriginModel,                 # trained language model
    tokenizer: AutoTokenizer,                # tokenizer 
    preprocess: clip = None,                 # processor of the image
    encoder: clip = None,                    # clip backbone
) -> None:
    ...
    for idx, item in tqdm(enumerate(annotations), total=len(annotations)):
        ...
        sim_pos = torch.nn.functional.cosine_similarity(image_features, avgall1.to(device), dim=2)
        sim_pos, _ = torch.topk(sim_pos, k = 5)
        sim_pos = torch.mean(sim_pos, dim = 1)

        sim_pos_1 = torch.nn.functional.cosine_similarity(image_features, avgall2.to(device), dim=2)
        sim_pos_1, _ = torch.topk(sim_pos_1, k = 5)
        sim_pos_1 = torch.mean(sim_pos_1, dim = 1)

        sim_pos += sim_pos_1

        _, i = torch.topk(sim_pos, k = 1)
...
def validation_msvd_msrvtt(
    ...
    output_dict = {}
    for key, tensor in eval_rt_dic.items():
        output_dict[key] = group_mean_every_5(tensor)
    eval_rt_dic = output_dict
...
def validation_sydney_ucm(
    args,
    inpath: str,                             # path of annotations file
    entities_text: List[str],                # entities texts of vocabulary
    texts_embeddings: torch.Tensor,          # entities embeddings of vocabulary
    modelnew: ClipCaptionModel,                 # trained language model
    modelold: ClipCaptionOriginModel,
    tokenizer: AutoTokenizer,                # tokenizer 
    preprocess: clip = None,                 # processor of the image
    encoder: clip = None,                    # clip backbone
) -> None:
...
def main(args) -> None:
    ...
    modelnew = ClipCaptionModel(args, args.continuous_prompt_length, args.clip_project_length, clip_hidden_size, gpt_type = args.language_model, k=args.k)
    modelnew.load_state_dict(torch.load(args.weight_path, map_location = device), strict=False)
    modelnew.to(device)
    modelold = ClipCaptionOriginModel(args, args.continuous_prompt_length, args.clip_project_length, 512, gpt_type = args.language_model, k=args.k)
    modelold.load_state_dict(torch.load("/home/liu/.workspace/IFCap/checkpoints_img_fea_best_1098_norm_prefix_0/coco/coco_prefix-004.pt"), strict=False)
    modelold.to(device)
    ...
    if args.name_of_datasets == 'nocaps': # nocaps
        if args.using_image_features:
            validation_nocaps(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)
        else:
            validation_nocaps(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)
    elif args.name_of_datasets in ['coco', "flickr30k"]: # coco, flickr30k
        if args.using_image_features:
            validation_coco_flickr30k(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)
        else:
            validation_coco_flickr30k(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)
    elif args.name_of_datasets in ['msvd', "msrvtt"]:
        if args.using_image_features:
            validation_msvd_msrvtt(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)
        else:
            validation_msvd_msrvtt(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)        
    elif args.name_of_datasets in ['sydney', "ucm"]:
        if args.using_image_features:
            validation_sydney_ucm(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)
        else:
            validation_sydney_ucm(args, inpath, entities_text, texts_embeddings, modelnew, modelold, tokenizer, preprocess, encoder)        

def group_mean_every_5(x: torch.Tensor) -> torch.Tensor:
    """
    将形状 [25,512] 的张量 x，
    按第 0 维每 5 行一组求平均，
    返回形状 [5,512] 的张量。
    """
    # 如果底层内存不连续，可先调用 contiguous()
    x = x.contiguous()            # 保证内存连续（可选）&#8203;:contentReference[oaicite:2]{index=2}
    # 重塑为 [5,5,512]
    x_grp = x.view(5, 5, 512)     # 或者 x.reshape(5,5,512) :contentReference[oaicite:3]{index=3}
    # 对组内部的 5 行求平均，得到 [5,512]
    return x_grp.mean(dim=1)      # 在 dim=1 上求均值&#8203;:contentReference[oaicite:4]{index=4}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name_of_datasets', default = 'nocaps', choices = ('coco', 'flickr30k', 'nocaps', "msvd", "msrvtt", "sydney", "ucm")) # coco
    ...
    parser.add_argument('--retrieved_info', type = str, default='caption_msrvtt_image_msrvtt_5_woclass.json')
