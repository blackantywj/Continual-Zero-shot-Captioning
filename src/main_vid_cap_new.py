...
# from ClipCap import ClipCaptionModel as ClipCaptionModelOri
...
def train(
    ...
    for epoch in range(epochs):
        ...
        if args.prefix == "msvd":
            cmd_msvd = [
                "bash", "scripts/eval_msvd_ifcap.sh",
                "train_msvd",
                str(0),
                "",
                "msvd",
            ...
            _ = subprocess.run(cmd_msvd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            cmd_flickr = [
        ...
        if args.prefix == "msrvtt":
            ...
            print(f"Running evaluation script for epoch {epoch} …")
            _ = subprocess.run(cmd_msrvtt, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)         
...
def main():
    ...
    parser.add_argument('--path_of_datasets', default = f'/home/liu/.workspace/IFCap/annotations/msrvtt/msrvtt_with_entities_cocoid_imgfeature.pickle') # coco_with_entities
    ...
    if args.frozen1 or args.frozen2:
        if 'msvd' in args.rt_path:
            ori_model_state_dict = torch.load("/home/liu/.workspace/IFCap/checkpoints_r32_alpha64_0_0/train_flickr30k/009.pt")
        if 'msrvtt' in args.rt_path:
            ori_model_state_dict = torch.load("/home/liu/.workspace/IFCap/checkpoints_img_fea_best_1098_norm_prefix_0/coco/coco_prefix-004.pt")
        if args.frozen2:
            for i in range(12):
