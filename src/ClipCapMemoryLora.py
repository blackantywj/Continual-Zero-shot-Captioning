...
class att_gt_n_rt(nn.Module):
    def __init__(
        ...
        self.transformer = Transformer(d_model, num_layers, num_heads, use_moe_lora = use_moe_lora)
...
class att_gt_n_rt_memory(nn.Module):
    def __init__(
        self,
        d_model: int,
        num_layers: int = 8,
        ...
        super(att_gt_n_rt_memory, self).__init__()
        self.use_moe_lora = use_moe_lora
        if use_fusion_layer:
            feature_fusion_layer = BiAttentionBlock(
                v_dim=d_model,
                l_dim=d_model,
                embed_dim=d_model * 4 // 2,
        ...
        self.transformer = MemoryTransformer(d_model, num_layers, num_heads, feature_fusion_layer = feature_fusion_layer, use_moe_lora = self.use_moe_lora)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        if use_moe_lora:
            self.to_queries = LoraLinear(base_layer=self.to_queries)
            self.to_keys_values = LoraLinear(base_layer=self.to_keys_values)

    #     self._reset_parameters()

    # def _reset_parameters(self):
    #     if self.use_moe_lora:
    #         nn.init.xavier_uniform_(self.to_queries.base_layer_.weight)
    #         # self.to_queries.base_layer_.bias.data.fill_(0)
    #         nn.init.xavier_uniform_(self.to_keys_values.base_layer_.weight)
    #         # self.to_keys_values.base_layer_.bias.data.fill_(0)
    #     else:
    #         nn.init.xavier_uniform_(self.to_queries.weight)
    #         # self.to_queries.bias.data.fill_(0)
    #         nn.init.xavier_uniform_(self.to_keys_values.weight)
    #         # self.to_keys_values.bias.data.fill_(0)
...
class TransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.attn = MultiHeadAttention(query_size, key_value_size, num_heads, bias = bias, dropout = dropout, use_moe_lora=use_moe_lora)
...
class MemoryTransformer(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_layers):
            layers.append(TransformerLayer(query_size, key_value_size, num_heads, mlp_ratio = mlp_ratio, act = act, norm_layer = norm_layer, use_moe_lora=use_moe_lora))        
    ...
    def forward(self, query: torch.Tensor, key_value: torch.Tensor = None, mask: torch.Tensor = None) -> torch.Tensor:
        ...
        for layer in self.fusion_layers:
            query, key_value = layer(query, key_value)
...
class MappingNetwork(nn.Module):
    ...
    def __init__(
        ...
        if use_moe_lora:
            self.linear = LoraLinear(base_layer=self.linear)
            self.rt_linear = LoraLinear(base_layer=self.rt_linear)
        ...
        self.crossatt = att_gt_n_rt(d_model, 1, num_heads, use_moe_lora = use_moe_lora)
        self.device = device
...
class T2II2TMemoryMappingNetwork(nn.Module):
    ...
    def __init__(
        ...
        if args.frozen2:
            ...
            lora_config = LoraConfig(
                r=args.r,                       # 降秩 r 32
                lora_alpha=args.lora_alpha,             # LoRA α 64
                target_modules=["c_attn",  # 注入到 self-attn QKV 投影
                                "c_proj", 
                                "mlp.c_fc", 
                                "mlp.c_proj"], # 注入到 self-attn 输出投影
                lora_dropout=0.05,          # dropout 保持率
                bias="none",               # 不加偏置
        ...
        self.mapping_network = MappingNetwork(clip_project_length, clip_hidden_size, continuous_length, self.gpt_hidden_size, num_layers, num_heads, k, args.device, use_moe_lora = args.use_moe_lora)
